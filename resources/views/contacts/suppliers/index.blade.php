                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-6 py-4">
                                <a href="{{ route('suppliers.index', array_merge(request()->all(), ['sort' => 'name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center group cursor-pointer hover:text-primary-600 dark:hover:text-white">
                                    Name
                            ...
                            <th scope="col" class="px-6 py-4">
                                <a href="{{ route('suppliers.index', array_merge(request()->all(), ['sort' => 'business_name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center group cursor-pointer hover:text-primary-600 dark:hover:text-white">
                                    Business
                    ...
                    @forelse($suppliers as $supplier)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-150">
                            <th scope="row" class="flex flex-col px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                ...
                                @if($supplier->email)
                                    <span class="text-xs text-gray-500 dark:text-gray-400 font-normal">{{ $supplier->email }}</span>
                                @endif
                            </th>
                            <td class="px-6 py-4">
                             ...
                             <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    ...
                                    @if($supplier->landline)
                                        <span class="text-xs text-gray-500">{{ $supplier->landline }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('suppliers.show', $supplier) }}" class="p-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 dark:hover:bg-blue-700 dark:focus:ring-blue-800 shadow transition-all hover:-translate-y-0.5" title="View">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    <a href="{{ route('suppliers.edit', $supplier) }}" class="p-2 text-white bg-yellow-400 rounded-lg hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 dark:focus:ring-yellow-900 shadow transition-all hover:-translate-y-0.5" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                     <form action="{{ route('suppliers.destroy', $supplier) }}" method="POST" data-confirm-message="Are you sure you want to delete this supplier?" class="inline">
                                        @csrf
                                        @method('DELETE')
