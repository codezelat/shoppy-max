    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                <div>
                    <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $reseller->name }}</h3>
                    <p class="text-xl text-gray-600 dark:text-gray-400 font-medium mt-1">{{ $reseller->business_name }}</p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <span class="bg-indigo-100 text-indigo-800 text-sm font-medium px-3 py-1 rounded-full dark:bg-indigo-900 dark:text-indigo-300">
                            Reseller
                        </span>
                        <span class="bg-gray-100 text-gray-800 text-sm font-medium px-3 py-1 rounded-full dark:bg-gray-700 dark:text-gray-300 flex items-center">
                         ...
                         @if($reseller->due_amount > 0)
                            <span class="bg-red-100 text-red-800 text-sm font-medium px-3 py-1 rounded-full dark:bg-red-900 dark:text-red-300">
                                Due: {{ number_format($reseller->due_amount, 2) }}
                            </span>
                        @else
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('resellers.edit', $reseller) }}" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800 flex items-center shadow-md transition-transform hover:scale-105">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        Edit Reseller
                    </a>
                    
                    <form action="{{ route('resellers.destroy', $reseller) }}" method="POST" class="inline-block" data-confirm-message="Are you sure you want to delete this reseller? This action cannot be undone.">
                        @csrf
                        @method('DELETE')
            ...
            <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 h-full hover:shadow-md transition-shadow">
                ...
                <ul class="space-y-4 text-gray-600 dark:text-gray-400">
                    <li class="flex items-center">
                        ...
                        <span class="text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded">{{ $reseller->mobile }}</span>
                    </li>
                     <li class="flex items-center">
                        <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">Landline:</span>
                        <span class="text-gray-900 dark:text-white">{{ $reseller->landline ?? '-' }}</span>
                    </li>
                     <li class="flex items-center">
                        <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">Email:</span>
                        <span class="text-gray-900 dark:text-white">{{ $reseller->email ?? '-' }}</span>
                    </li>
                </ul>
            </div>
                ...
                <h4 class="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                 ...
                 <ul class="space-y-4 text-gray-600 dark:text-gray-400">
                    <li class="flex items-start">
                        ...
                        <span class="text-gray-900 dark:text-white">{{ $reseller->address }}</span>
                    </li>
                     <li class="flex items-start">
                        <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">City:</span>
                        <span class="text-gray-900 dark:text-white">{{ $reseller->city }}</span>
                    </li>
                     <li class="flex items-start">
                        <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">Country:</span>
                        <span class="text-gray-900 dark:text-white">{{ $reseller->country }}</span>
                    </li>

                    @if($reseller->country === 'Sri Lanka' || $reseller->province)
                    <li class="flex items-start">
                        <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">Province:</span>
                         <span class="text-gray-900 dark:text-white">{{ $reseller->province ?? '-' }}</span>
                    </li>
                    @endif

                    @if($reseller->country === 'Sri Lanka' || $reseller->district)
                    <li class="flex items-start">
                         <span class="font-semibold w-24 text-gray-900 dark:text-gray-300">District:</span>
                         <span class="text-gray-900 dark:text-white">{{ $reseller->district ?? '-' }}</span>
                    </li>
                    @endif
                 </ul>
            </div>
        ...
        <div class="flex justify-end">
             <a href="{{ route('resellers.index') }}" class="inline-flex items-center px-5 py-2.5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 transition-colors">
