            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($cities as $city)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $city->city_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $city->postal_code }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $city->district }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('cities.show', $city) }}" class="text-indigo-600 hover:text-indigo-900 mr-2">View</a>
                                <a href="{{ route('cities.edit', $city) }}" class="text-blue-600 hover:text-blue-900 mr-2">Edit</a>
                                <form action="{{ route('cities.destroy', $city) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">No cities found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
