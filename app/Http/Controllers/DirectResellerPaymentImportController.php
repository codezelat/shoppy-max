...
use App\Exports\DirectResellerPaymentTemplateExport;
use Illuminate\Support\Facades\DB;
use Carbon\Carbon;

class DirectResellerPaymentImportController extends Controller
{
    private const SESSION_KEY = 'direct_import_preview_data';

    public function show()
    {
        return view('direct-resellers.payments.import');
    }
    ...
    public function preview(Request $request)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            // Validate Reseller
            $reseller = Reseller::direct()->find($resellerId);
            $errors = [];
        ...
        session([self::SESSION_KEY => $previewData]);

        return view('direct-resellers.payments.import', compact('previewData', 'validRowsCount', 'hasErrors'));
    }

    public function store(Request $request)
    {
        $previewData = session(self::SESSION_KEY);

        if (!$previewData) {
            return redirect()->route('direct-reseller-payments.import.show')->with('error', 'Session expired. Please upload the file again.');
        }
        ...
        DB::transaction(function () use ($previewData, &$count) {
            foreach ($previewData as $row) {
                ...
                // Update Reseller Due
                $reseller = Reseller::direct()->find($row['reseller_id']);
                if ($reseller) {
        ...
        return redirect()->route('direct-reseller-payments.index')->with('success', "Successfully imported $count payments.");
    }
}
